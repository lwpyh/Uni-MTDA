...
def cal_acc(loader, netF,netB,netC,netCplus, label_list,label_map):
    start_test = True
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            outputs = netCplus(outputs,label_list)
            if start_test:
...
def train_target(args):
    ...
    for k, v in netB.named_parameters():
        if args.lr_decay2 > 0:
            ...
            v.requires_grad = False
    netCplus = network.Dy_Linear(50,50).cuda()
    for p, q in netCplus.named_parameters():
    ...
    while iter_num < max_iter:
        ...
        outputs_test1 = netCplus(outputs_test,label_list)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == 'VISDA-C':
                acc_s_te, acc_list = cal_acc(dset_loaders['test'], netF, netB, netC,netCplus, label_list,label_map)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name, iter_num, max_iter,
                                                                            acc_s_te) + '\n' + acc_list
            else:
                acc_s_te, _ = cal_acc(dset_loaders['test'], netF, netB, netC,netCplus, label_list,label_map)
