...
def cal_acc_oda(loader, netF, netB, netC, netC_ex, args, class_len, flag=False):
    ...
    if flag:
        ...
        pre_class_acc1 = pre_class_acc[start:-1].tolist()
        pre_class_acc1.append(pre_class_acc[-1])
        # per_class_acc1 = np.array(per_class_acc1.extend(pre_class_acc[-1]))
        H_score = np.mean(pre_class_acc1[:-1]) * pre_class_acc1[-1] * 2 / (
                    pre_class_acc1[-1] + np.mean(pre_class_acc1[:-1]))
        print("per_class_acc", pre_class_acc1, np.mean(pre_class_acc1), np.mean(pre_class_acc1[:-1]),
              pre_class_acc1[-1], H_score)
        return np.mean(pre_class_acc1[:-1]), np.mean(pre_class_acc1), pre_class_acc1[-1]

def cal_acc_oda1(loader, netF, netB, netC, netC_ex, args, class_len, flag=False):
    start_test = True
    per_class_num = np.zeros((args.class_num + 1))
    per_class_correct = np.zeros((args.class_num + 1)).astype(np.float32) + 1e-9
    with torch.no_grad():
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            print(len(label_list1), label_list1)

            label_map2 = {}
            rev_label_map2 = {}
            mem_label2 = obtain_label1(dset_loaders['test2'], netF, netB, netC2, netC_ex2, iter_num, args)
            ...
            print(len(label_list2), label_list2)
        ...
        if args.cls_par > 0:
            ...
            classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs_0, dummytargets_0) * 0.3
            classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs_1, dummytargets_1) * 0.3
            classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs, dummytargets) * 0.3
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            netC_ex2.eval()
            if True:
                acc_os1, acc_os2, acc_unknown = cal_acc_oda1(dset_loaders['test'], netF, netB, netC, netC_ex,
                ...
                acc_os1, acc_os2, acc_unknown = cal_acc_oda1(dset_loaders['test1'], netF, netB, netC1, netC_ex1, args,
                                                           tar_classes_len1, True)
                log_str1 = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.name2, iter_num,
                                                                                            max_iter, acc_os2,
                                                                                                 acc_os1,acc_unknown)
                print(log_str1)
                args.out_file.write(log_str1 + '\n')
                acc_os1, acc_os2, acc_unknown = cal_acc_oda1(dset_loaders['test2'], netF, netB, netC2, netC_ex2,
...
def obtain_label1(loader, netF, netB, netC, netC_ex, iter_num, args):
    ...
    if iter_num == 0:
        threshold = 0.0
    elif min(cls_count[:-1])/max(cls_count[:-1]) < 0.00001 :
...
if __name__ == "__main__":
    ...
    if args.dset == 'office-home':
        names = ['Product_U', 'Real_World_U', 'Art_U', 'Clipart_U']
