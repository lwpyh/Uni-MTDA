...
def cal_acc_oda(loader, netF, netB, netC, netC_ex):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
            ...
            dummylogit = netC_ex(netB(netF(inputs)))
    ...
    # predict[predict == threshold] = args.class_num
    matrix = confusion_matrix(all_label, torch.squeeze(predict).float())
...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        dummytargets_new = torch.zeros(outputs_source.size(0),outputs_source.size(1)).cuda()
        for i in range(len(dummpyoutputs)):
            nowlabel = labels_source[i]
            a = dummpyoutputs[i,:nowlabel]
            b = dummpyoutputs[i,nowlabel+1:]
            dummytargets_new[i] = torch.cat((a,b),dim=0)
        dummytargets = torch.ones_like(labels_source) * (args.class_num - 1)
        ...
        classifier_loss += nn.CrossEntropyLoss()(dummytargets_new, dummytargets) * 1.0
    ...
    torch.save(best_netF, osp.join(args.output_dir_src, "source_F7.pt"))
    torch.save(best_netB, osp.join(args.output_dir_src, "source_B7.pt"))
    torch.save(best_netC, osp.join(args.output_dir_src, "source_C7.pt"))
    torch.save(best_netC_ex, osp.join(args.output_dir_src, "source_C_ex7.pt"))
...
def test_target(args):
    ...
    netC = network.feat_classifier(type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck).cuda()
    netC_ex = network.feat_classifier(type=args.layer, class_num=args.extra_class, bottleneck_dim=args.bottleneck).cuda()
    args.modelpath = args.output_dir_src + '/source_F7.pt'
    netF.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + '/source_B7.pt'
    netB.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + '/source_C7.pt'
    netC.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + '/source_C_ex7.pt'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--extra_class', type=int, default=3)
    ...
    if args.dset == 'office-home':
        names = ['Clipart', 'Product', 'Real_World', 'Art']
        args.class_num = 50
    if args.dset == 'office':
