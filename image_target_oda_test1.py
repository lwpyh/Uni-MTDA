...
def cal_acc(loader, netF, netB, netC, netC_ex, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            outputs_ex = F.sigmoid(dummylogit)
    ...
    if flag:
        ...
        # ent = torch.sum(-all_output_ex * torch.log(all_output_ex + args.epsilon), dim=1) / np.log(args.class_num)
        # ent = ent.float().cpu()
        # initc = np.array([[0], [1]])
        ...
        #     iidx = 1
        a = args.class_num * np.ones(len(all_label), )
        for i in range(all_output_ex.size(0)):
            a[i] = all_output_ex[i, predict[i]]
        predict[np.where(a < 0.5)[0]] = args.class_num
...
def train_target(args):
    ...
    netC_ex2 = network.ResClassifier_MME(class_num=args.extra_class, input_dim=256, norm=False).cuda()

    modelpath = args.output_dir_src + '/source_F4.pt'
    netF.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_B4.pt'
    netB.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_C4.pt'
    netC2.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_C_ex4.pt'
    netC_ex2.load_state_dict(torch.load(modelpath))
    # netC.eval()
    netC_ex2.weight_norm()
    ...
    while iter_num < max_iter:
        ...
        if args.cls_par > 0:
            ...
            out_open = outputs_test_ex3.view(outputs_test_ex3.size(0), 1, -1)
            ent_open = open_entropy(out_open)
            outputs_test3 = outputs_test3[pred2 < args.class_num, :]
            pred2 = pred2[pred2 < args.class_num]

            # print("tar_idx2", mem_label2)

            classifier_loss = nn.CrossEntropyLoss()(outputs_test3 ,pred2)
            classifier_loss *= args.cls_par
            classifier_loss += ent_open
            # classifier_loss +=  0.2 * nn.CrossEntropyLoss()(dummpyoutputs[pred2 < args.class_num], dummytargets[pred2< args.class_num])
...
def obtain_label(loader, netF, netB, netC, netC_ex, args):
    ...
    all_output_ex = F.sigmoid(all_output_ex)
    ...
    for i in range(all_output_ex.size(0)):
        a[i] = all_output_ex[i,predict[i]]
    known_idx = np.where(a > 0.5)[0]
    print(known_idx, len(known_idx), b)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--extra_class', type=int, default=50)
