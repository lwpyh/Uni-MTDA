...
def miloss(softmax_out_known,softmax_out_known_s,C):
    P = (softmax_out_known.unsqueeze(2) * softmax_out_known_s.unsqueeze(1)).sum(dim=0)
    P = ((P + P.t()) / 2) / P.sum()
    Pi = P.sum(dim=1).view(C, 1).expand(C, C)
    Pj = P.sum(dim=0).view(1, C).expand(C, C)
    return torch.sum(P * (torch.log(Pi) + torch.log(Pj) - torch.log(P)))
...
def image_train_da(resize_size=256, crop_size=224, alexnet=False):
    if not alexnet:
        ...
        normalize = Normalize(meanfile='./ilsvrc_2012_mean.npy')
    return transforms.Compose([
        transforms.RandomResizedCrop(resize_size, scale=(0.2, 1.)),
...
def data_load(args):
    ...
    dset_loaders["target"] = DataLoader(dsets["target"], batch_size=train_bs, shuffle=True, num_workers=args.worker,
                                        drop_last=False)
    dsets["target_da"] = ImageList(txt_tar, transform=image_train_da())
    dset_loaders["target_da"] = DataLoader(dsets["target_da"], batch_size=train_bs, shuffle=True,
    ...
    dset_loaders["target1"] = DataLoader(dsets["target1"], batch_size=train_bs, shuffle=True, num_workers=args.worker,
                                         drop_last=False)
    dsets["target1_da"] = ImageList(txt_tar1, transform=image_train_da())
    dset_loaders["target1_da"] = DataLoader(dsets["target1_da"], batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
    ...
    dset_loaders["target2"] = DataLoader(dsets["target2"], batch_size=train_bs, shuffle=True, num_workers=args.worker,
                                         drop_last=False)
    dsets["target2_da"] = ImageList(txt_tar2, transform=image_train_da())
    dset_loaders["target2_da"] = DataLoader(dsets["target2_da"], batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
...
def cal_acc_oda(loader, netF, netB, netC, netC_ex, label_list, flag=False):
    start_test = True
    with torch.no_grad():
...
def train_target(args):
    ...
    modelpath = args.output_dir_src + '/source_F2.pt'
    netF.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_B2.pt'
    netB.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_C2.pt'
    netC.load_state_dict(torch.load(modelpath))
    netC1.load_state_dict(torch.load(modelpath))
    netC2.load_state_dict(torch.load(modelpath))
    # modelpath = args.output_dir_src + '/source_C_ex2.pt'
    ...
    while iter_num < max_iter:
        try:
            ...
            inputs_test_da, _, _1 = iter_test_da.next()
            inputs_test1, _, tar_idx1 = iter_test1.next()
            inputs_test_da1, _, _2 = iter_test_da1.next()
            inputs_test2, _, tar_idx2 = iter_test2.next()
            inputs_test_da2, _, _3 = iter_test_da2.next()
        except:
            torch.manual_seed(iter_num)
            iter_test = iter(dset_loaders["target"])
            inputs_test, _, tar_idx = iter_test.next()
            torch.manual_seed(iter_num)
            iter_test_da = iter(dset_loaders["target_da"])
            inputs_test_da, _ = iter_test_da.next()
            torch.manual_seed(iter_num)
            iter_test1 = iter(dset_loaders["target1"])
            inputs_test1, _, tar_idx1 = iter_test1.next()
            torch.manual_seed(iter_num)
            iter_test_da1 = iter(dset_loaders["target1_da"])
            inputs_test_da1, _ = iter_test_da1.next()
            torch.manual_seed(iter_num)
            iter_test2 = iter(dset_loaders["target2"])
            inputs_test2, _, tar_idx2 = iter_test2.next()
            torch.manual_seed(iter_num)
            iter_test_da2 = iter(dset_loaders["target2_da"])
            inputs_test_da2, _ = iter_test_da2.next()
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            label_map1 = {}
            rev_label_map1 = {}
            mem_label1 = obtain_label(dset_loaders['test1'], netF, netB, netC1, netC_ex, args)
            ...
            label_map2 = {}
            rev_label_map2 = {}
            mem_label2 = obtain_label(dset_loaders['test2'], netF, netB, netC2, netC_ex, args)
            ...
            # acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders['test2'], netF, netB, netC2, netC_ex, label_list2, True)
        ...
        inputs_test_da = inputs_test_da.cuda()
        inputs_test_da1 = inputs_test_da1.cuda()
        inputs_test_da2 = inputs_test_da2.cuda()

        input_test = torch.cat([inputs_test, inputs_test1, inputs_test2], dim=0)
        input_test_da = torch.cat([inputs_test_da, inputs_test_da1, inputs_test_da2], dim=0)
        iter_num += 1
        lr_scheduler(optimizer, iter_num=iter_num, max_iter=max_iter)
        # bs*2048 , bs*1000
        features_test = netB(netF(input_test))
        features_test_da = netB(netF(input_test_da))
        ...
        outputs_test_da1 = netC((features_test_da[:len(tar_idx), ...]), label_list)
        outputs_test_da2 = netC1(features_test_da[len(tar_idx):len(tar_idx) + len(tar_idx1), ...], label_list1)
        outputs_test_da3 = netC2(features_test_da[len(tar_idx) + len(tar_idx1):, ...], label_list2)
        ...
        pseudo_label1 = torch.softmax(outputs_test1.detach() / args.T, dim=1)
        pseudo_label2 = torch.softmax(outputs_test2.detach() / args.T, dim=1)
        pseudo_label3 = torch.softmax(outputs_test3.detach() / args.T, dim=1)
        _, targets_u1 = torch.max(pseudo_label1, dim=-1)
        _, targets_u2 = torch.max(pseudo_label2, dim=-1)
        _, targets_u3 = torch.max(pseudo_label3, dim=-1)
        classifier_loss_t_s1 = (F.cross_entropy(outputs_test_da1, targets_u1, reduction='none')).mean()
        classifier_loss_t_s2 = (F.cross_entropy(outputs_test_da2, targets_u2, reduction='none')).mean()
        classifier_loss_t_s3 = (F.cross_entropy(outputs_test_da3, targets_u3, reduction='none')).mean()
        ...
        if args.cls_par > 0:
            ...
            # criterion = loss.SCELoss(alpha=args.alpha, beta=args.beta, num_classes=args.class_num)
            ...
            classifier_loss += classifier_loss_t_s1 + classifier_loss_t_s2 + classifier_loss_t_s3

            # classifier_loss = criterion(outputs_test1, pred) + criterion(outputs_test2, pred1) + criterion(outputs_test3, pred2)
            classifier_loss *= args.cls_par
            # classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs, dummytargets)

            # classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs_0, dummytargets_0)
        ...
        if args.ent:
            ...
            softmax_out_s = nn.Softmax(dim=1)(outputs_test_da1)
            C = outputs_test_da1.size()[1]
            entropy_loss = miloss(softmax_out, softmax_out_s, C)
            # entropy_loss = torch.mean(loss.Entropy(softmax_out))
            softmax_out1 = nn.Softmax(dim=1)(outputs_test2)
            softmax_out_s1 = nn.Softmax(dim=1)(outputs_test_da2)
            C1 = outputs_test_da2.size()[1]
            entropy_loss1 = miloss(softmax_out1, softmax_out_s1, C1)
            # entropy_loss1 = torch.mean(loss.Entropy(softmax_out1))
            softmax_out2 = nn.Softmax(dim=1)(outputs_test3)
            softmax_out_s2 = nn.Softmax(dim=1)(outputs_test_da3)
            C2 = outputs_test_da3.size()[1]
            entropy_loss2 = miloss(softmax_out2, softmax_out_s2, C2)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.name == args.name3:
                ...
                args.out_file.write(log_str1 + '\n')
                acc2, _ = cal_acc(dset_loaders['test1'], netF, netB, netC1, netC_ex, label_list1, label_map1)
                log_str2 = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name2, iter_num, max_iter, acc2)
                args.out_file.write(log_str2 + '\n')
                acc3, _ = cal_acc(dset_loaders['test1'], netF, netB, netC2, netC_ex, label_list2, label_map2)
                log_str3 = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name3, iter_num, max_iter, acc3)
...
def obtain_label(loader, netF, netB, netC, netC_ex, args):
    ...
    maxdummy, _ = torch.max(all_output_ex.clone(), dim=1)
    all_output_ex = maxdummy.view(-1, 1)
    all_output1 = torch.cat((all_output, all_output_ex), dim=1)
    all_output1 = nn.Softmax(dim=1)(all_output1)
    K = all_output1.size(1)
    # for i in range(all_output1.size(0)):
    #     if all_output1[i,-1] < 0.8:
    #         all_output1[i,-1] = 0.0
    _, predict = torch.max(all_output1, 1)
    cls_count = np.eye(K)[predict].sum(axis=0)
    all_output1 = all_output1.mean(dim=0)
    all_output = nn.Softmax(dim=1)(all_output)
    print("all_output1", all_output1)
    print("cls_count", cls_count, min(cls_count[:-1]) / (cls_count[-1]), np.sum(cls_count),
          np.mean(cls_count[:-1]) / cls_count[-1], np.sum(cls_count[:-1])/np.sum(cls_count))
    _, predict = torch.max(all_output, 1)
    ...
    for round in range(1):
        ...
        for i in range(len(initc)):
            if initc[i].all() == np.zeros(all_fea.shape[1], ).all():
                initc[i] = 10 * np.ones(all_fea.shape[1], )
        dd = cdist(all_fea, initc[labelset], args.distance)
        pred_label = dd.argmin(axis=1)
        pred_label = labelset[pred_label]
    ...
    # gmm.fit(all_fea)
    # prediction = gmm.predict(all_fea)
    # acc1 = np.sum(prediction == all_label.float().numpy()) / len(all_fea)
    # log_str1 = 'Accuracy1 = {:.2f}% -> {:.2f}%'.format(acc * 100, acc1 * 100)
...
def obtain_label1(loader, netF, netB, netC, netC_ex, args):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for _ in range(len(loader)):
    ...
    print(all_output1)
    # ent = torch.sum(-all_output1[:-1] * torch.log(all_output1[:-1] + args.epsilon)) #/ np.log(args.class_num)
    # ent = ent.float().cpu()
    # ent = ent.mean()
    # print("ent", ent)
    #
    ...
    cls_count = np.eye(K)[predict].sum(axis=0)
    predict1 = predict
    print("cls_count", cls_count, min(cls_count[:-1]) / (cls_count[-1]), np.sum(cls_count), np.mean(cls_count[:-1])/cls_count[-1], np.sum(cls_count[:-1])/np.sum(cls_count))
    # from sklearn.cluster import KMeans
    # kmeans = KMeans(2, random_state=0).fit(ent.reshape(-1, 1))
    # labels = kmeans.predict(ent.reshape(-1, 1))
    #
...
if __name__ == "__main__":
    ...
    parser.add_argument("--T", type=float, default=0.5, help='parameter for temperature2')
    ...
    if args.dset == 'office-home':
        ...
        args.tar_classes1 = [i for i in range(65)]
