...
def train_target(args):
    ...
    modelpath = args.output_dir_src + '/source_F7.pt'
    netF.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_B7.pt'
    netB.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_C7.pt'
    netC.load_state_dict(torch.load(modelpath))
    modelpath = args.output_dir_src + '/source_C_ex7.pt'
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            label_map = {}
            rev_label_map = {}
            mem_label = obtain_label1(dset_loaders['test'], netF, netB, netC, netC_ex, args)
            label_list = list(set(mem_label))
            label_list.sort()
            label_list = torch.arange(0, args.class_num + 1)
            ...
            acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders['test'], netF, netB, netC, netC_ex, label_list, True)
        ...
        outputs_test1 = netC(features_test, label_list[:-1])
        outputs_test_ex1 = netC_ex(features_test)
        if args.cls_par > 0:
            ...
            dummytargets_0 = torch.ones_like(pred) * args.class_num
            outputs_test1 = torch.cat((outputs_test1, outputs_test_ex1), dim=1)

            classifier_loss = nn.CrossEntropyLoss()(outputs_test1, pred)
            classifier_loss *= args.cls_par
            classifier_loss += nn.CrossEntropyLoss()(dummpyoutputs_0, dummytargets_0) * 0.1
        ...
        if iter_num % interval_iter == 100000000 or iter_num == max_iter:
            netF.eval()
            netB.eval()
            netC.eval()
            netC_ex.eval()
            ...
            if args.name == args.name1:
                ...
                acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders['test'], netF, netB, netC, netC_ex, label_list,
                                                            True)
                log_str1 = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.name, iter_num,
                                                                                                 max_iter, acc_os2,
                                                                                                 acc_os1, acc_unknown)
...
def obtain_label(loader, netF, netB, netC, netC_ex, args):
    ...
    possibility = all_output1.mean(dim=0)
    ...
    all_output = nn.Softmax(dim=1)(all_output)
    ent = (torch.sum(-all_output1 * torch.log(all_output1 + args.epsilon), dim=1) / np.log(args.class_num + 1)).mean(
        dim=0)
    ent1 = (torch.sum(-all_output_n * torch.log(all_output_n + args.epsilon), dim=1) / np.log(args.class_num)).mean(
        dim=0)
    ent2 = (torch.sum(-possibility1 * torch.log(possibility1 + args.epsilon), dim=1) / np.log(args.class_num)).mean(
        dim=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--da', type=str, default='oda', choices=['uda', 'pda'])
    ...
    if args.dset == 'office-home':
        # names = ['Product','Real_World', 'Clipart', 'Art']
        names = ['Art', 'Real_World', 'Product', 'Clipart']
