...
def data_load():
    ## prepare data
    train_bs = 64
    txt_tar = open("./data/office/dslr_list.txt").readlines()
    txt_test = open("./data/office/dslr_list.txt").readlines()
    dataset_train = ImageList_idx(txt_tar, transform=image_train())
    train_loader = DataLoader(dataset_train, batch_size=train_bs, shuffle=True, num_workers=4,
                                        drop_last=False)
    dataset_valid = ImageList_idx(txt_test, transform=image_test())
    valid_loader = DataLoader(dataset_valid, batch_size=train_bs * 3, shuffle=False, num_workers=4,
                                      drop_last=False)
...
if __name__ == "__main__":
    ...
    model_base = CNN(32, 3, args.channels, 31, args.layers)
    netB = feat_bootleneck(type="bn", feature_dim=model_base.out_feature,
                                   bottleneck_dim=256).cuda()
    netC = feat_classifier(type="wn", class_num=31, bottleneck_dim=256).cuda()
    modelpath = "ckpt/source/uda/office/A" + '/source_B.pt'
    netB.load_state_dict(torch.load(modelpath))
    modelpath = "ckpt/source/uda/office/A" + '/source_C.pt'
