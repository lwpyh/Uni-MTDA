...
def data_load(args):
    ...
    train_cifar, val_cifar = get_dataset("cifar10")
    train_stl,val_stl = get_dataset("STL10")
    dset_loaders["source_tr"] = DataLoader(train_cifar, batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
    # dsets["source_te"] = ImageList(val_cifar, transform=image_test())
    dset_loaders["source_te"] = DataLoader(val_cifar, batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
    # dsets["test"] = ImageList(txt_test, transform=image_test())
    dset_loaders["test"] = DataLoader(train_stl, batch_size=train_bs * 2, shuffle=True, num_workers=args.worker,
                                      drop_last=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='cifar-stl',
