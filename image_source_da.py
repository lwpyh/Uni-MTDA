...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        inputs_source, inputs_source_da, inputs_source_da1, labels_source = inputs_source.cuda(), inputs_source_da.cuda(), inputs_source_da1.cuda(), labels_source.cuda()
        outputs_source = netC(netB(netF(inputs_source)))
        outputs_source_da = netC(netB(netF(inputs_source_da)))
        outputs_source_da1 = netC(netB(netF(inputs_source_da1)))

        classifier_loss = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source,
                                                                                                   labels_source)

        p_clean, p_aug1, p_aug2 = F.softmax(
            outputs_source, dim=1), F.softmax(
            outputs_source_da, dim=1), F.softmax(
            outputs_source_da1, dim=1)
        ...
        # print("2", (F.kl_div(p_mixture, p_clean, reduction='batchmean') +
        #               F.kl_div(p_mixture, p_aug1, reduction='batchmean')) /2.)
        # + F.kl_div(p_mixture, p_aug2, reduction='batchmean')) / 3.)
        optimizer.zero_grad()
    ...
    torch.save(best_netF, osp.join(args.output_dir_src, "source_F2.pt"))
    torch.save(best_netB, osp.join(args.output_dir_src, "source_B2.pt"))
    torch.save(best_netC, osp.join(args.output_dir_src, "source_C2.pt"))
...
def test_target(args):
    ...
    args.modelpath = args.output_dir_src + '/source_F2.pt'
...
if __name__ == "__main__":
    ...
    if args.dset == 'office-home':
        names = ['Real_World', 'Art', 'Clipart', 'Product']
        args.class_num = 25
