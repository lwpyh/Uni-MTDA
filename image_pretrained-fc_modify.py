...
def cal_acc(loader, net,netC, label_list,label_map):
    start_test = True
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            outputs = netC(features,label_list)
            if start_test:
...
def train_target(args):
    ...
    temp_w = netF.state_dict()['fc.weight']
    temp_b = netF.state_dict()['fc.bias']

    netC = network.Dy_Linear(2048,1000).cuda()
    state_dict = netC.state_dict()
    state_dict['weight'] = temp_w
    state_dict['bias'] = temp_b
    netC.load_state_dict(state_dict)
    # netC = network.Dy_Linear(1000,1000).cuda()
    ...
    while iter_num < max_iter:
        ...
        outputs_test1 = netC(features_test,label_list)
        ...
        if args.ent:
            ...
            if args.gent:
                msoftmax = nn.Softmax(dim=1)(outputs_test).mean(0)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            netF.eval()
            netC.eval()
            acc, ment = cal_acc(dset_loaders['test'], netF,netC, label_list,label_map)
...
if __name__ == "__main__":
    ...
    # 输出路径
    args.output_dir = osp.join(args.output, args.da, args.dset+"-otsu-gent")
    args.name = args.dset
